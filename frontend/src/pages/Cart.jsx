...
const Cart = () => {
  // CHANGE 1: 'clearCart' ni ikkada add chesanu
  ...
  const placeOrderInDB = async (paymentId) => {
    ...
    try {
      const orderData = {
        ...
        paymentResult: { id: paymentId, status: 'success' }, 
      ...
      if (response.ok) {
        // CHANGE 2: Order success avvagane Cart ni empty chestunna
        clearCart(); 
  ...
  return (
    <div style={{ maxWidth: '600px', margin: '0 auto', padding: '20px', color: '#fff' }}>
      <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom: '30px'}}>
        ...
        {/* Optional: Manual Clear Cart Button */}
        <button onClick={clearCart} style={{background:'transparent', border:'1px solid #dc3545', color:'#dc3545', padding:'5px 10px', borderRadius:'5px', cursor:'pointer'}}>Clear Cart</button>
      </div>
